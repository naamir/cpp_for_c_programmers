...
Graph::Graph(int nodes)
{
    ...
    graph_2d_array = new bool*[num_nodes]; // creates a dynamic array using heap memory
    srand(time(0)); // start the seed for random number generator
    for (int i = 0; i < num_nodes; i++)
    {
        graph_2d_array[i] = new bool[num_nodes]; // adds the 2nd dimention of that array
    }
}
...
void Graph::edgeGen()
{
    for (int i = 0; i < Graph::num_nodes; i++)
    {
        for (int j = 0; j < Graph::num_nodes; j++)
        {
            if (i == j) graph_2d_array[i][j] = false; // assuming NO loops
            // fixing density to be 19%
            // NOTE: the condition (rand < 19) will return true or false...
            else graph_2d_array[i][j] = graph_2d_array[j][i] = (Graph::get_random_number() < 50);
